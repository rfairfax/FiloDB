...
use range_aware_regex::RangeAwareRegexQuery;
use tantivy::{
    query::{
...
fn parse_prefix_query<'a>(
    ...
    let query = query_with_field_and_value(schema, default_field, &column, |field, prefix| {
        // Turn into a regex since there's no native prefix query type
        let text = format!("{}.*", regex::escape(&text));
